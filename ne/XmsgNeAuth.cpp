...
void XmsgNeAuth::handle(shared_ptr<XscChannel> channel, SptrXitp trans, shared_ptr<XmsgNeAuthReq> req)
{
	...
	shared_ptr<XmsgImHlrCfgXmsgNeN2hAddr> cfgNe = XmsgNeAuth::findXmsgNeCfg(req->cgt());
	if (cfgNe == nullptr)
	{
	...
	if (req->ext().find("sub-x-msg-client-estb-event") != req->ext().end()) 
		XmsgNeMgr::instance()->addSubClientEstbEventGroup(nu);
	if (req->ext().find("sub-x-msg-client-disc-event") != req->ext().end()) 
		XmsgNeMgr::instance()->addSubClientDiscEventGroup(nu);
	auto old = XmsgNeMgr::instance()->add(nu);
	shared_ptr<XmsgNeAuthRsp> rsp(new XmsgNeAuthRsp());
	rsp->set_cgt(XmsgImHlrCfg::instance()->cfgPb->cgt());
	trans->end(rsp);
	if (old == nullptr)
		return;
...
shared_ptr<XmsgImHlrCfgXmsgNeN2hAddr> XmsgNeAuth::findXmsgNeCfg(const string& cgt)
{
	for (int i = 0; i < XmsgImHlrCfg::instance()->cfgPb->n2h_size(); ++i) 
	{
		auto ne = XmsgImHlrCfg::instance()->cfgPb->n2h(i);
		if (ne.cgt() != cgt)
			continue;
		shared_ptr<XmsgImHlrCfgXmsgNeN2hAddr> cfgNe(new XmsgImHlrCfgXmsgNeN2hAddr());
		cfgNe->CopyFrom(ne);
		return cfgNe;
	}
	return nullptr;
